...
<div id="teachingSystemModal" class="fixed inset-0 z-50 hidden bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
    <div class="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden transform transition-all animate-fade-in-up" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        ...
        <form id="teachingSystemForm" onsubmit="saveTeachingSystem(event)" class="p-5 space-y-4">
            @csrf
            <!-- Section 1: Teaching Mode -->
            <div>
                <label class="block text-xs font-black text-slate-700 dark:text-slate-300 font-arabic mb-2">
                    <i class="fas fa-chalkboard-teacher text-brand-primary me-1"></i> نمط وطريقة التدريس
                </label>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <label class="relative flex flex-col p-3 rounded-2xl border cursor-pointer transition-all teaching-mode-card hover:border-brand-primary/60 border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                        <input type="radio" name="teaching_mode" value="online" class="sr-only" {{ ($teachingMode ?? 'online') === 'online' ? 'checked' : '' }} onchange="highlightSelectedCards()">
                        <div class="flex items-center gap-2 mb-1">
                            <i class="fas fa-laptop text-brand-primary text-sm"></i>
                            <span class="text-xs font-black text-slate-800 dark:text-slate-200 font-arabic">أونلاين بالكامل</span>
                        </div>
                        <span class="text-[10px] text-slate-400 leading-tight">حصص بث مباشر، تسجيلات وامتحانات</span>
                    </label>

                    <label class="relative flex flex-col p-3 rounded-2xl border cursor-pointer transition-all teaching-mode-card hover:border-brand-primary/60 border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                        <input type="radio" name="teaching_mode" value="in_person" class="sr-only" {{ ($teachingMode ?? '') === 'in_person' ? 'checked' : '' }} onchange="highlightSelectedCards()">
                        <div class="flex items-center gap-2 mb-1">
                            <i class="fas fa-school text-emerald-600 text-sm"></i>
                            <span class="text-xs font-black text-slate-800 dark:text-slate-200 font-arabic">سنتر وحضوري</span>
                        </div>
                        <span class="text-[10px] text-slate-400 leading-tight">مجموعات وقاعات وحضور بالباركود</span>
                    </label>

                    <label class="relative flex flex-col p-3 rounded-2xl border cursor-pointer transition-all teaching-mode-card hover:border-brand-primary/60 border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                        <input type="radio" name="teaching_mode" value="hybrid" class="sr-only" {{ ($teachingMode ?? '') === 'hybrid' ? 'checked' : '' }} onchange="highlightSelectedCards()">
                        <div class="flex items-center gap-2 mb-1">
                            <i class="fas fa-sync-alt text-amber-500 text-sm"></i>
                            <span class="text-xs font-black text-slate-800 dark:text-slate-200 font-arabic">نظام هجين</span>
                        </div>
                        <span class="text-[10px] text-slate-400 leading-tight">سنتر + مراجعات وحصص أونلاين</span>
                    </label>
                </div>
            </div>
