...
@section('content')
    <div class="mb-4">
        <h2 class="fw-bold text-dark">{{ __('instructor::schedules.edit_title') }}</h2>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('instructor.schedules.index') }}">{{ __('instructor::schedules.title') }}</a></li>
                <li class="breadcrumb-item active">{{ __('instructor::sidebar.edit') }}</li>
            </ol>
        </nav>
    </div>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('instructor.schedules.update', $schedule) }}" method="POST">
                        ...
                        <div class="d-flex gap-2 pb-5 mt-4">
                            <button type="submit" class="btn btn-primary px-5 py-2 fw-bold text-white shadow-sm rounded-pill border-0">{{ __('instructor::schedules.update_schedule') }}</button>
