...
<div class="login-page-wrap min-h-screen bg-slate-50/50 flex justify-center items-center p-4 lg:p-8"
    ...
    <div class="login-card w-full max-w-md bg-white rounded-[40px] shadow-2xl shadow-slate-200/60 p-8 lg:p-10 border border-slate-100 animate-fade-in-up">
        ...
        <div class="text-center mb-8">
            <div class="d-inline-flex justify-center items-center rounded-full w-20 h-20 mb-4 mx-auto text-3xl font-bold shadow-sm" style="background: rgba(124, 58, 237, 0.08); color: var(--brand-violet);">
                <i class="fas fa-user-friends"></i>
            </div>
            <h1 class="text-2xl font-bold text-slate-900 mb-2 font-arabic tracking-tight">
                {{ __('parent::auth.login_title') }}
            </h1>
            <p class="text-slate-500 text-sm font-arabic font-light">
                {{ $tenant->name ?? __('parent::auth.login_subtitle') }}
            </p>
        </div>
        ...
        <form action="{{ route('parent.login.submit') }}" method="POST" class="space-y-5" x-data="{ showPassword: false }">
            @csrf
            ...
            <div class="pt-4">
                <button type="submit"
                    ...
                    <span class="font-arabic">{{ __('دخول') }}</span>
        ...
        <div class="mt-8 pt-6 border-t border-slate-100 flex justify-center">
            <a href="{{ route('center.login') }}" class="group flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-violet-600 transition-colors">
                ...
                <span class="font-arabic">{{ __('العودة لصفحة دخول المركز') }}</span>
            </a>
        </div>
    </div>
</div>
