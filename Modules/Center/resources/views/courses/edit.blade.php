...
@section('title', __('center::courses.edit_title', ['title' => $course->title]))

@section('panel-content')
    <x-ui.page-header
        title="{{ __('center::courses.edit_title', ['title' => $course->title]) }}"
        subtitle="{{ __('center::courses.edit_subtitle') ?? 'تحديث بيانات الدورة الدراسية ومواعيد الجلسات' }}"
        :breadcrumb="[
            ['label' => __('center::dashboard.title'), 'url' => route('center.dashboard')],
            ['label' => __('center::courses.title'), 'url' => route('center.courses.index')],
            ['label' => __('center::courses.edit')]
        ]"
    >
        <x-slot name="actions">
            ...
            <form action="{{ route('center.courses.update', $course->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <div class="border-b border-brand-border dark:border-slate-800 pb-3 mb-4 flex items-center gap-2">
                        ...
                        <h3 class="text-base font-bold text-slate-900 dark:text-slate-100 font-inter">تعديل بيانات الدورة</h3>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $course->title) }}" 
                               class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 text-slate-900 dark:text-slate-100 focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-sm @error('title') border-rose-500 @enderror">
                        @error('title')
                    ...
                    <div>
                        ...
                        <select name="instructor_id" 
                                class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 text-slate-900 dark:text-slate-100 focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-sm @error('instructor_id') border-rose-500 @enderror">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <div>
                            ...
                            <input type="number" name="sessions_count" value="{{ old('sessions_count', $course->sessions_count) }}" 
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <label class="cursor-pointer">
                                    <input type="radio" name="status" id="status_draft" value="draft" class="peer hidden" {{ old('status', $course->status) == 'draft' ? 'checked' : '' }}>
                                ...
                                <label class="cursor-pointer">
                                    <input type="radio" name="status" id="status_published" value="published" class="peer hidden" {{ old('status', $course->status) == 'published' ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        @if($course->image)
                            <div class="mb-3">
                                <img src="{{ Storage::url($course->image) }}" alt="Current Image" class="w-24 h-24 object-cover rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                            </div>
                        @endif
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3" 
                                  class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 text-slate-900 dark:text-slate-100 focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-sm @error('description') border-rose-500 @enderror">{{ old('description', $course->description) }}</textarea>
                        @error('description')
                            <p class="text-rose-500 text-xs mt-1.5 flex items-center gap-1"><i class="fas fa-exclamation-circle"></i>{{ $message }}</p>
                        @enderror
                    </div>
                                ...
                                <div class="schedule-item p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/40 relative group transition-all">
                                    <div class="flex items-center justify-between mb-3 border-b border-slate-200/80 dark:border-slate-700/80 pb-2">
                                        <h6 class="text-xs font-bold text-emerald-600 dark:text-emerald-400 font-inter flex items-center gap-1.5">
                                            ...
                                            <span>{{ __('center::schedules.item_number') }} <span class="schedule-index">{{ $index + 1 }}</span></span>
                                        </h6>
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                        ...
                                        <div>
                                            ...
                                            <input type="time" name="schedules[{{ $index }}][end_time]" value="{{ \Carbon\Carbon::parse($schedule->end_time)->format('H:i') }}" class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-xs">
                                        </div>
                                    </div>
                                </div>
                            @endforeach
