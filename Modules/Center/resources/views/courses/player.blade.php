                ...
                <div class="card-body p-0">
                    ...
                    @elseif($lesson->type == 'quiz')
                        <div class="p-5 text-center">
                            ...
                            @if($quiz)
                                <p class="text-muted">درجة النجاح: {{ $quiz->passing_score }}% | الوقت: {{ $quiz->duration_minutes ?? 'غير محدد' }} دقيقة</p>
                                <a href="{{ route('center.quizzes.show', $quiz) }}" class="btn btn-primary btn-lg rounded-pill px-5">ابدأ الاختبار الآن</a>
                            @else
                                <p class="text-danger">لم يتم إعداد أسئلة هذا الاختبار بعد.</p>
                            @endif
                        </div>
                    @elseif($lesson->type == 'assignment')
                        <div class="p-5 text-center">
                            ...
                            @if($assignment)
                                ...
                                <a href="{{ route('center.assignments.show', $assignment) }}" class="btn btn-success btn-lg rounded-pill px-5">عرض وتفاصيل التكليف</a>
                            @else
                                <p class="text-danger">لم يتم إعداد تفاصيل هذا التكليف بعد.</p>
                            @endif
                        </div>
                    @endif
                </div>
            ...
            <div class="card shadow-sm mt-4 border-0 rounded-4">
                <div class="card-header bg-white border-0 p-0 overflow-hidden">
                    <ul class="nav nav-pills nav-fill bg-light border-bottom p-1" id="lessonTabs" role="tablist">
                        <li class="nav-item">
                            <button class="nav-link active rounded-pill py-2" id="about-tab" data-bs-toggle="tab" data-bs-target="#about" type="button">عن الدرس</button>
                        </li>
                        <li class="nav-item">
                    ...
                    <div class="tab-content" id="lessonTabsContent">
                        <div class="tab-pane fade show active" id="about">
                            ...
                            <div class="text-muted">
                                {!! nl2br(e($lesson->description ?? 'لا يوجد وصف متاح لهذا الدرس حالياً.')) !!}
                            </div>
                        </div>
                            ...
                            <div class="row g-3">
                                ...
                                @empty
                                    <div class="col-12 text-center text-muted py-4">
                                        ...
                                        <p>لا توجد ملفات مرفقة بهذا الكورس حالياً.</p>
                                    </div>
                                @endforelse
                            </div>
                        </div>
