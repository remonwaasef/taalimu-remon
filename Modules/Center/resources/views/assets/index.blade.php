...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted small mb-0">جرد العهود والأصول مرتبة حسب القاعات والمرافق</p>
        </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
            <div class="card-header bg-light bg-opacity-50 py-3 border-0 d-flex justify-content-between align-items-center">
                <h6 class="fw-bold text-dark mb-0">
                    ...
                    {{ $classroomName == '---' ? 'عُهد غير مسندة لقاعة' : $classroomName }}
                                            ...
                                            <ul class="dropdown-menu border-0 shadow">
                                                ...
                                                <li>
                                                    <form action="{{ route('center.assets.destroy', $asset) }}" method="POST" onsubmit="return confirm('{{ __('center::assets.confirm_delete') }}')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button class="dropdown-item text-danger">حذف القطعة</button>
                                                    </form>
                                                </li>
                                            </ul>
                                        </div>
    ...
    @empty
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-body p-5 text-center">
                ...
                <a href="{{ route('center.assets.create') }}" class="btn btn-primary rounded-pill mt-3 px-4">
                    إضافة أول عهدة الآن
                </a>
            </div>
        </div>
