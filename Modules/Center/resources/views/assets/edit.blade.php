        ...
        <div class="col-12">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active text-primary" aria-current="page">{{ isset($asset) ? 'تعديل' : 'إضافة جديد' }}</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="card-body p-4">
            <form action="{{ isset($asset) ? route('center.assets.update', $asset) : route('center.assets.store') }}" method="POST">
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary px-5 rounded-pill shadow-sm fw-bold">حفظ</button>
                    <a href="{{ route('center.assets.index') }}" class="btn btn-light px-4 rounded-pill border">إلغاء</a>
                </div>
            </form>
        </div>
    </div>
