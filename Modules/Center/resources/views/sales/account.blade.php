...
<div class="row g-4">
        ...
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-body p-4">
                <div class="row align-items-end g-3">
                    <div class="col-md-8">
                        <label class="form-label fw-bold text-dark"><i class="fas fa-search me-1"></i>{{ __('center::messages.blade_0560') }}</label>
                    ...
                    <div class="col-md-4">
                        <div id="quickStats" class="d-none animate__animated animate__fadeIn">
                            <div class="bg-danger bg-opacity-10 rounded-pill p-3 border border-danger border-opacity-25 d-flex justify-content-between align-items-center">
                                <span class="text-danger fw-bold">{{ __('center::messages.blade_0562') }}</span>
            ...
            <div class="col-lg-7">
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-header bg-white border-0 py-3 d-flex justify-content-between align-items-center">
                        <h5 class="fw-bold mb-0 text-dark"><i class="fas fa-book me-2"></i>{{ __('center::messages.blade_0563') }}</h5>
                        ...
                        <div class="table-responsive">
                            <table class="table align-middle mb-0">
                                <thead class="bg-light">
                                    <tr>
                                        <th class="border-0 ps-4">{{ __('center::messages.blade_0564') }}</th>
                                        <th class="border-0">{{ __('center::messages.blade_0565') }}</th>
                                        <th class="border-0">{{ __('center::messages.blade_0566') }}</th>
                                    </tr>
                                </thead>
            ...
            <div class="col-lg-5">
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-header bg-white border-0 py-3">
                        <h5 class="fw-bold mb-0 text-danger"><i class="fas fa-exclamation-triangle me-2"></i>{{ __('center::messages.blade_0567') }}</h5>
                    </div>
    ...
    <div id="emptyState" class="col-lg-12 text-center py-5">
        <div class="bg-white rounded-4 shadow-sm p-5 d-inline-block" style="max-width: 500px;">
            ...
            <p class="text-muted">{{ __('center::messages.blade_0570') }}</p>
        </div>
    </div>
</div>
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.courses.length > 0) {
                        ...
                        tableBody.innerHTML = '<tr><td colspan="3" class="text-center py-4 text-muted">{{ __('center::messages.blade_0571') }}</td></tr>';
                    }
                    ...
                    if (data.unpaid_invoices.length > 0) {
                            ...
                            list.innerHTML += `
                                <div class="bg-white border rounded-4 p-3 mb-3 d-flex justify-content-between align-items-center hover-shadow transition-all">
                                    ...
                                    <div class="d-flex flex-column gap-2">
                                        <button class="btn btn-success btn-sm rounded-pill px-3" onclick="collectDebt(${inv.id}, ${inv.remaining}, ${studentId})">{{ __('center::messages.blade_0572') }}</button>
                                        <a href="/sales/${inv.id}" target="_blank" class="btn btn-outline-light text-dark btn-sm rounded-pill px-3 border small">{{ __('center::messages.blade_0573') }}</a>
                                    </div>
                                </div>`;
                        });
                    } else {
                        list.innerHTML = `
                            <div class="text-center py-4">
                                ...
                                <h6 class="fw-bold text-success">{{ __('center::messages.blade_0574') }}</h6>
                                <p class="text-muted small mb-0">{{ __('center::messages.blade_0575') }}</p>
                            </div>`;
                    }
                }
            });
