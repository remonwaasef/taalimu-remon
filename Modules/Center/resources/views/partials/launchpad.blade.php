@php
    $steps = [
        'education_system' => ['icon' => 'fa-graduation-cap', 'color' => 'indigo', 'route' => 'center.settings.index'],
        'instructor' => ['icon' => 'fa-chalkboard-teacher', 'color' => 'indigo', 'route' => 'center.instructors.create'],
        'course' => ['icon' => 'fa-book-open', 'color' => 'indigo', 'route' => 'center.courses.create'],
        'student' => ['icon' => 'fa-user-graduate', 'color' => 'indigo', 'route' => 'center.students.create'],
        'attendance' => ['icon' => 'fa-clipboard-check', 'color' => 'indigo', 'route' => 'center.attendance.index'],
    ];
...
<div class="mb-8 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs relative overflow-hidden font-inter transition-all duration-200">
    ...
    <div class="h-1 w-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
        <div class="h-full bg-gradient-to-r from-indigo-600 via-indigo-500 to-purple-600 rounded-full transition-all duration-700 ease-out" style="width: {{ $launchpadProgress }}%;"></div>
    </div>
    ...
    <div class="p-5 sm:p-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-5 border-b border-slate-100 dark:border-slate-800">
            <div class="flex items-center gap-3.5 min-w-0">
                <div class="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-lg shrink-0 border border-indigo-100 dark:border-indigo-800/40 shadow-xs">
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2.5 flex-wrap">
                        ...
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 border border-indigo-200/60 dark:border-indigo-800/40">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                @else
                    <form action="{{ route('center.demo.seed', ['tenant' => $tenant->domain ?? app('tenant')?->domain]) }}" method="POST" id="demoDataForm" class="m-0">
                        @csrf
                        <button type="submit" class="px-3 py-1.5 rounded-xl text-xs font-bold text-indigo-600 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/40 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 border border-indigo-200/60 dark:border-indigo-800/40 transition-colors flex items-center gap-1.5">
            ...
            @foreach($steps as $key => $data)
                ...
                <div class="
                    ...
                    {{ $isCompleted 
                        ...
                        : ($isCurrent 
                            ? 'bg-indigo-50/50 dark:bg-indigo-950/30 border-2 border-indigo-600 shadow-xs' 
                ...
                ">
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span class="text-[10px] font-bold {{ $isCompleted ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400' }}">
                                {{ $loop->iteration }}/5
                            </span>
                        </div>
                    ...
                    <div class="pt-3 mt-auto">
                        ...
                        @elseif($key === 'education_system')
                            <button
                                type="button" 
                                class="w-full py-1.5 px-3 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 {{ $isCurrent ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-xs' : 'bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200' }}"
                        ...
                        @else
                            <a
                                ...
                                class="w-full py-1.5 px-3 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 no-underline {{ $isCurrent ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-xs' : 'bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200' }}"
                            >
                                <span>{{ __('center::dashboard.launchpad.action') }}</span>
                                <i class="fas fa-arrow-left text-[9px] rtl:rotate-0 ltr:rotate-180"></i>
                            </a>
...
<x-ui.modal id="education-system-modal" size="md">
    <x-slot name="header">
        <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-xl bg-indigo-50 dark:bg-indigo-950/50 flex items-center justify-center text-indigo-600 dark:text-indigo-300">
    ...
    <form action="{{ route('center.settings.apply-template', ['tenant' => $tenant->domain ?? app('tenant')?->domain]) }}" method="POST" id="educationSystemForm" class="space-y-4">
        ...
        <div class="bg-indigo-50/80 dark:bg-indigo-950/40 border border-indigo-200/70 dark:border-indigo-800/40 rounded-xl text-xs p-3 text-indigo-700 dark:text-indigo-300 flex items-start gap-2.5">
