@php
    $steps = [
        ...
        'instructor' => ['icon' => 'fa-chalkboard-teacher', 'color' => 'sky', 'route' => 'center.instructors.create'],
        'course' => ['icon' => 'fa-book-open', 'color' => 'amber', 'route' => 'center.courses.create'],
        'student' => ['icon' => 'fa-user-graduate', 'color' => 'emerald', 'route' => 'center.students.create'],
        'attendance' => ['icon' => 'fa-clipboard-check', 'color' => 'purple', 'route' => 'center.attendance.index'],
    ];
...
<div class="mb-8 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs relative overflow-hidden font-inter transition-all duration-200">
    ...
    <div class="h-1 w-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
        <div class="h-full bg-gradient-to-r from-brand-primary via-emerald-400 to-sky-400 rounded-full transition-all duration-700 ease-out" style="width: {{ $launchpadProgress }}%;"></div>
    </div>
    ...
    <div class="p-5 sm:p-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-5 border-b border-slate-100 dark:border-slate-800">
            <div class="flex items-center gap-3.5 min-w-0">
                <div class="w-10 h-10 rounded-xl bg-brand-50 dark:bg-brand-950/50 text-brand-primary dark:text-brand-300 flex items-center justify-center text-lg shrink-0 border border-brand-100 dark:border-brand-800/40 shadow-xs">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                @else
                    <form action="{{ route('center.demo.seed', ['tenant' => $tenant->domain ?? app('tenant')?->domain]) }}" method="POST" id="demoDataForm" class="m-0">
                        @csrf
                        <button type="submit" class="px-3 py-1.5 rounded-xl text-xs font-bold text-brand-primary dark:text-brand-300 bg-brand-50 dark:bg-brand-950/40 hover:bg-brand-100 dark:hover:bg-brand-900/40 border border-brand-200/60 dark:border-brand-800/40 transition-colors flex items-center gap-1.5">
            ...
            @foreach($steps as $key => $data)
                ...
                <div class="
                    ...
                    {{ $isCompleted 
                        ...
                        : ($isCurrent 
                            ? 'bg-brand-50/50 dark:bg-brand-950/30 border-2 border-brand-primary shadow-xs' 
                ...
                ">
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span class="text-[10px] font-bold {{ $isCompleted ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400' }}">
                                {{ $loop->iteration }}/5
                            </span>
                        </div>
...
<x-ui.modal id="education-system-modal" size="md">
    <x-slot name="header">
        <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-xl bg-brand-50 dark:bg-brand-950/50 flex items-center justify-center text-brand-primary dark:text-brand-300">
