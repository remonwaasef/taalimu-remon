...
<head>
    ...
    {{-- Dashboard-aligned typography: Cairo (AR) + Inter (EN/FR) --}}
    ...
    <link rel="stylesheet" href="{{ asset('css/landing-v3.css') }}?v={{ filemtime(public_path('css/landing-v3.css')) }}">
    ...
    <style>
        ...
        .dark .v3-tab[aria-selected="true"] {
            background-color: #2e8b83;
            color: white;
        }
        /* Micro-animations */
        @keyframes floatSlow {
        ...
        /* Comparison slider */
        .v3-comparison-slider {
            position: relative;
            overflow: hidden;
        }
        .v3-comparison-handle {
            position: absolute;
            top: 0;
            bottom: 0;
            width: 4px;
...
<body class="antialiased overflow-x-hidden transition-colors duration-300">
    <div class="min-h-screen flex flex-col">
        @include('landing.v3.header')

        <main class="flex-grow">
            @include('landing.v3.hero')
            @include('landing.v3.workflow')
            @include('landing.v3.problem')
            @include('landing.v3.outcomes')
            @include('landing.v3.showcase')
            @include('landing.v3.qr-attendance')
            @include('landing.v3.whatsapp')
            @include('landing.v3.finance')
            @include('landing.v3.teacher')
            @include('landing.v3.center')
            @include('landing.v3.ecosystem')
            @include('landing.v3.comparison')
            @include('landing.v3.trust')
            @include('landing.v3.pricing')
            @include('landing.v3.final-cta')
        </main>

        @include('landing.v3.sticky-cta')
        @include('landing.v3.footer')
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            window.addEventListener('scroll', function () {
                ...
                if (!marks[50] && p >= 0.5) { marks[50] = true; TaalimuTrack('v3_scroll_depth', { depth: 50 }); }
                if (!marks[90] && p >= 0.9) { marks[90] = true; TaalimuTrack('v3_scroll_depth', { depth: 90 }); }
            }, { passive: true });
        });
    </script>
