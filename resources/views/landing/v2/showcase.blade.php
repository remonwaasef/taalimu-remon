<section id="showcase" class="v2-section bg-white border-y border-[color:var(--color-border-subtle)]" aria-labelledby="v2-showcase-title">
    <div class="v2-container">
        <div class="text-center max-w-2xl mx-auto mb-10 lg:mb-12" data-reveal>
            <span class="v2-eyebrow mb-4">{{ __('landing-v2.showcase.badge') }}</span>
        ...
        <div x-data="{ tab: 'dashboard' }" data-reveal style="--reveal-delay: 100ms;">
            {{-- Tabs --}}
            <div class="flex items-center gap-1.5 overflow-x-auto justify-start sm:justify-center p-1.5 rounded-[var(--card-radius)] bg-[color:var(--color-secondary-50)] border border-[color:var(--color-border-subtle)] max-w-full sm:max-w-2xl mx-auto mb-8 lg:mb-10"
                ...
                @foreach(['dashboard' => __('landing-v2.showcase.tabs.dashboard'), 'students' => __('landing-v2.showcase.tabs.students'), 'reports' => __('landing-v2.showcase.tabs.reports'), 'whatsapp' => __('landing-v2.showcase.tabs.whatsapp')] as $key => $label)
                    <button type="button" role="tab" class="v2-tab" :aria-selected="tab === '{{ $key }}' ? 'true' : 'false'" @click="tab = '{{ $key }}'">
                        {{ $label }}
                    </button>
                @endforeach
            </div>
            ...
            <div class="max-w-4xl mx-auto">
                ...
                <div x-show="tab === 'dashboard'" role="tabpanel" aria-label="{{ __('landing-v2.showcase.tabs.dashboard') }}">
                    <figure class="v2-product-frame">
                        ...
                        <figcaption class="p-5 lg:p-6 bg-white border-t border-[color:var(--color-border-subtle)]">
                    ...
                    <div class="v2-preview">
                        ...
                        <div class="divide-y divide-[color:var(--color-border-subtle)]">
                            @foreach([['أحمد علي', 'A', 'الصف 10', 90, 'success'], ['فاطمة أحمد', 'F', 'الصف 9', 75, 'success'], ['محمد خالد', 'M', 'الصف 9', 60, 'warning'], ['سارة يوسف', 'S', 'الصف 10', 92, 'success']] as [$name, $initial, $grade, $pct, $state])
                                <div class="flex items-center gap-3 px-4 lg:px-6 py-3.5 bg-white">
                                    ...
                                    <span class="hidden sm:flex items-center gap-2 w-32" aria-hidden="true">
                                        <span class="flex-1 h-1.5 rounded-full bg-[color:var(--color-secondary-100)] overflow-hidden">
                                            <span class="block h-full rounded-full {{ $state === 'warning' ? 'bg-amber-400' : 'bg-[color:var(--color-success-400)]' }}" style="width: {{ $pct }}%;"></span>
                                        </span>
                            ...
                            <svg viewBox="0 0 200 72" class="w-full h-auto" role="img" aria-label="{{ __('landing-v2.showcase.reports_desc') }}">
                                <defs>
                                    <linearGradient id="v2line" x1="0" y1="0" x2="1" y2="0">
                                        <stop offset="0%" stop-color="#4F46E5"/><stop offset="100%" stop-color="#22C55E"/>
                                    </linearGradient>
                                </defs>
                                ...
                                <circle cx="175" cy="16" r="4" fill="#4F46E5"/>
                            </svg>
                        </div>
                        <div class="v2-card p-5 flex flex-col justify-between gap-4">
                            ...
                            <div class="space-y-3">
                                @foreach([[85, 'bg-[color:var(--color-primary-500)]'], [64, 'bg-[color:var(--color-primary-300)]'], [42, 'bg-emerald-400']] as [$w, $color])
                                    <div class="h-2 rounded-full bg-[color:var(--color-secondary-100)] overflow-hidden" aria-hidden="true">
                ...
                <div x-show="tab === 'whatsapp'" x-cloak role="tabpanel" aria-label="{{ __('landing-v2.showcase.tabs.whatsapp') }}">
                    <div class="max-w-md mx-auto v2-wa-bubble">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between gap-2 mb-1">
                                    <span class="text-xs font-extrabold text-[color:var(--color-success-800)]">Taalimu</span>
                                ...
                                <span class="flex items-center justify-end gap-1 mt-2 text-[10px] font-bold text-[color:var(--color-success-700)]">
