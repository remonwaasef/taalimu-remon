    ...
    >
        <div class="max-w-4xl mx-auto text-center">
            ...
            <div class="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-slate-50 border border-slate-100 mb-10 transition-transform hover:scale-105 duration-300">
                ...
                <span class="text-[12px] font-bold text-slate-600 uppercase tracking-widest">{{ __('landing.hero.badge') }}</span>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-5 justify-center mb-24">
                <a href="{{ route('register') }}" class="group relative bg-[#22c55e] text-white px-10 py-5 rounded-2xl font-bold text-lg shadow-xl shadow-green-500/10 hover:shadow-green-500/25 transition-all active:scale-95">
            ...
            <div class="mt-32 py-10 border-t border-slate-100">
                <p class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-10">{{ __('landing.hero.ratings') ?? 'Trusted by Modern Educators' }}</p>
                <div class="flex flex-wrap justify-center gap-12 lg:gap-24">
                    ...
                    <div class="group">
                        <div class="text-3xl lg:text-4xl font-extrabold text-[#0f172a] group-hover:text-[#22c55e] transition-colors duration-300">{{ $stat['value'] }}</div>
                        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">{{ $stat['label'] }}</div>
                    </div>
                    @endforeach
                </div>
            </div>
