<!-- Social Proof / Features Dark Section -->
<section id="features" class="py-24 bg-[#0B1120] relative overflow-hidden">
        ...
        <div class="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div data-animate>
                <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 mb-8">
                    <span class="text-xs font-bold text-emerald-400 uppercase tracking-widest">{{ __('landing.features.badge') }}</span>
                </div>
                <h2 class="text-3xl md:text-4xl font-black text-white mb-6 tracking-tight leading-tight">
                ...
                <div class="space-y-6">
                    @php
                        $featuresData = [
                            ['icon' => 'fa-users', 'color' => 'text-emerald-400', 'bg' => 'bg-emerald-500/10'],
                            ['icon' => 'fa-calendar-check', 'color' => 'text-blue-400', 'bg' => 'bg-blue-500/10'],
                            ['icon' => 'fa-credit-card', 'color' => 'text-violet-400', 'bg' => 'bg-violet-500/10'],
                            ['icon' => 'fa-chart-pie', 'color' => 'text-amber-400', 'bg' => 'bg-amber-500/10'],
                        ];
                    @endphp
                    @foreach(__('landing.features.items') as $index => $item)
                        ...
                        <div class="flex gap-4 group">
                            ...
                            <div>
                                <h3 class="text-white font-bold mb-1 group-hover:text-emerald-400 transition-colors">{{ $item['title'] }}</h3>
            ...
            <div class="grid grid-cols-2 gap-4" data-stagger>
                @foreach(__('landing.features.items') as $index => $item)
                    @if($index >= 4) @break @endif
                    @php $data = $featuresData[$index] ?? $featuresData[0]; @endphp
                    <div class="bg-[#111827] rounded-2xl p-6 border border-white/5 hover:border-white/10 transition-all group hover:-translate-y-1">
                        ...
                        <h4 class="text-white font-bold text-sm mb-2">{{ $item['title'] }}</h4>
