...
<div class="min-h-screen bg-slate-50/50 flex justify-center items-center p-4 lg:p-8" 
    ...
    <div class="w-full max-w-md bg-white rounded-[40px] shadow-2xl shadow-slate-200/60 p-8 lg:p-10 border border-slate-100 animate-fade-in-up">
        ...
        <div class="text-center mb-8">
            <div class="d-inline-flex justify-center items-center rounded-full w-20 h-20 mb-4 mx-auto text-3xl font-bold shadow-sm" style="background: rgba(16, 185, 129, 0.08); color: var(--brand-emerald);">
                <?php echo e(substr($tenant->name ?? 'C', 0, 1)); ?>

            </div>
            <h1 class="text-2xl font-bold text-slate-900 mb-2 font-arabic tracking-tight">
                <?php echo e($tenant->name ?? __('auth.login.title')); ?>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                         <h3 class="text-sm font-bold text-red-800 font-arabic mb-1">
                            <?php echo e(__(__('center::messages.blade_0155'))); ?>
        ...
        <form action="<?php echo e(route('center.login.submit')); ?>" method="POST" class="space-y-5" x-data="{ showPassword: false }">
            <?php echo csrf_field(); ?>
            
            <div class="space-y-1.5">
                <label class="text-[11px] font-bold text-slate-500 px-1 font-arabic uppercase tracking-wider"><?php echo e(__('auth.login.email_or_phone')); ?></label>
            ...
            <div class="space-y-1.5">
                <div class="flex justify-between items-center px-1">
                    <label class="text-[11px] font-bold text-slate-500 font-arabic uppercase tracking-wider"><?php echo e(__('auth.login.password')); ?></label>
                    <button type="button" @click="showPassword = !showPassword" class="text-[9px] font-black transition-colors uppercase tracking-widest text-emerald-600 hover:text-emerald-500">
                        <span x-text="showPassword ? (<?php echo e(app()->getLocale() == 'ar' ? '\'إخفاء\'' : '\'HIDE\''); ?>) : (<?php echo e(app()->getLocale() == 'ar' ? '\'إظهار\'' : '\'SHOW\''); ?>)"></span>
                    </button>
                </div>
            ...
            <div class="pt-4">
                <button type="submit" 
                    ...
                    <span class="font-arabic"><?php echo e(__('auth.login.login_button')); ?></span>
                    <svg class="w-4 h-4 text-white/80 group-hover:translate-x-1 <?php echo e(app()->getLocale() == 'ar' ? 'group-hover:-translate-x-1' : ''); ?> transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="<?php echo e(app()->getLocale() == 'ar' ? 'M10 19l-7-7m0 0l7-7m-7 7h18' : 'M14 5l7 7m0 0l-7 7m7-7H3'); ?>"></path>
                    </svg>
                </button>
            </div>
        </form>
        ...
        <div class="relative my-6">
            ...
            <div class="relative flex justify-center text-xs">
                <span class="px-3 bg-white text-slate-400 font-arabic font-semibold"><?php echo e(__('or')); ?></span>
            </div>
        </div>
        ...
        <a href="<?php echo e(route('auth.google')); ?>" 
           class="w-full h-12 flex items-center justify-center gap-3 border-2 border-slate-200 rounded-[20px] text-sm font-bold text-slate-700 bg-white hover:bg-slate-50 hover:border-slate-300 transition-all duration-300 group">
            ...
            <span class="font-arabic"><?php echo e(__('Sign in with Google')); ?></span>
        </a>

        <div class="mt-8 pt-6 border-t border-slate-100 flex justify-center">
            <a href="<?php echo e(route('home')); ?>" class="group flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-emerald-500 transition-colors">
                ...
                <span class="font-arabic"><?php echo e(__('landing.navbar.home')); ?></span>
            </a>
        </div>
    </div>
</div>
