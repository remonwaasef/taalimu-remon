...
<?php $__env->startSection('page-title', __('instructor::scanner.smart_attendance', ['title' => $course->title])); ?>
<?php $__env->startSection('page-subtitle', __('instructor::scanner.guide_camera')); ?>

<?php $__env->startSection('page-actions'); ?>
    <a href="<?php echo e(route('instructor.dashboard')); ?>" class="btn btn-glass shadow-sm">
        <i class="fas fa-arrow-right me-2"></i> <?php echo e(__('instructor::dashboard.back_to_dashboard')); ?>
        ...
        <div class="col-md-8 text-center">
                ...
                <div class="card-body p-0 position-relative">
                    ...
                    <div id="scanner-status" class="position-absolute top-50 start-50 translate-middle w-100 p-4 d-none" style="z-index: 10;">
                        <div class="alert alert-success shadow-lg rounded-pill animate__animated animate__pulse">
                            <i class="fas fa-check-circle me-2"></i> <span id="status-text"><?php echo e(__('instructor::scanner.attendance_marked')); ?></span>
                        </div>
                    </div>
                </div>

                <div class="card-footer bg-white border-0 py-4">
                    <?php if($activeSchedule = $schedule): ?>
                        <div class="alert alert-info small mb-0 rounded-pill">
                            <i class="fas fa-clock me-2"></i> <?php echo e(__('instructor::scanner.today_session')); ?> 
                            <strong><?php echo e(\Carbon\Carbon::parse($activeSchedule->start_time)->format('h:i A')); ?></strong>
                        </div>
                        <input type="hidden" id="schedule_id" value="<?php echo e($activeSchedule->id); ?>">
                    <?php else: ?>
                        <div class="alert alert-warning small mb-0 rounded-pill">
                            <i class="fas fa-exclamation-triangle me-2"></i> <?php echo e(__('instructor::scanner.no_session_today')); ?>

                        </div>
                    <?php endif; ?>
                    
                    <button id="toggle-camera" class="btn btn-primary rounded-pill px-5 mt-3 shadow">
                        <i class="fas fa-camera me-2"></i> <?php echo e(__('instructor::scanner.start_camera')); ?>
            ...
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header bg-transparent border-0 pt-4 px-4 text-start">
                    <h6 class="fw-bold mb-0"><?php echo e(__('instructor::scanner.recent_scans')); ?></h6>
                </div>
                <div class="card-body p-0">
                    <div id="recent-scans" class="list-group list-group-flush">
                        <div class="text-center py-4 text-muted small"><?php echo e(__('instructor::scanner.no_recent_scans')); ?></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function processScan(qrIdentifier) {
        if (!scheduleId) {
            showStatus("<?php echo e(__('instructor::scanner.no_active_session')); ?>", "danger");
            isProcessing = false;
            return;
        }

        fetch(`<?php echo e(route('instructor.scan', $course->id)); ?>`, {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                const statusMsg = data.already_marked ? "<?php echo e(__('instructor::scanner.already_marked')); ?>" : "<?php echo e(__('instructor::scanner.marked_success')); ?>";
        ...
        .catch(error => {
            ...
            showStatus("<?php echo e(__('instructor::scanner.server_error')); ?>", "danger");
        })
        .finally(() => {
            setTimeout(() => {
                isProcessing = false;
        ...
        item.innerHTML = `
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <small class="text-muted"><?php echo e(__('instructor::scanner.remaining_sessions', ['balance' => '${balance ?? "--"}'])); ?></small>
                </div>
                <div class="d-flex gap-2">
                    <a href="${whatsappUrl}" target="_blank" class="btn btn-sm btn-success rounded-pill px-3">
                        <i class="fab fa-whatsapp"></i> <?php echo e(__('instructor::scanner.notify')); ?>

                    </a>
                    <span class="badge ${already ? 'bg-warning' : 'bg-success'} rounded-pill d-flex align-items-center">
                        ${already ? '<?php echo e(__('instructor::scanner.already_marked')); ?>' : (already === false ? '<?php echo e(__('instructor::scanner.active')); ?>' : '--')}
                    </span>
                </div>
            </div>
        `;
    ...
    document.getElementById('toggle-camera').addEventListener('click', function() {
        if (html5QrScanner) {
            ...
            html5QrScanner = null;
            this.innerHTML = '<i class="fas fa-camera me-2"></i> <?php echo e(__('instructor::scanner.start_camera')); ?>';
        } else {
            ...
            this.innerHTML = '<i class="fas fa-stop me-2"></i> <?php echo e(__('instructor::scanner.stop_camera')); ?>';
        }
    });
