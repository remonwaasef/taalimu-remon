...
document.addEventListener('alpine:init', () => {
    Alpine.data('registrationForm', (config) => ({
        ...
        currentStep: <?php echo e($errors->hasAny(['name', 'email', 'phone', 'password']) ? 2 : 1); ?>,
        showPassword: false,
        password: '',
...
<div class="min-h-[85vh] bg-slate-50/50 flex justify-center p-4 lg:p-8 mesh-gradient-soft noise-overlay register-page-offset" 
     x-data="registrationForm({
        selectedPlan: <?php echo e(Js::from(old('plan', request('plan', $packages->firstWhere('is_default', true)?->slug ?? $packages->first()?->slug ?? '')))); ?>,
        billingCycle: <?php echo e(Js::from(old('billing_cycle', request('cycle', 'monthly')))); ?>,
        ...
        accountType: <?php echo e(Js::from(old('account_type', $accountType))); ?>,
        selectedCurrency: <?php echo e(Js::from(old('currency', request('currency', session('suggested_currency', 'EGP'))))); ?>,
        userCountry: '<?php echo e(session('user_country_code', '')); ?>'
     })"
    ...
    <div class="w-full transition-all duration-500 bg-white rounded-3xl shadow-2xl shadow-blue-900/5 overflow-hidden border border-slate-100/50 animate-fade-in-up md:backdrop-blur-xl relative"
        ...
        <div class="bg-white p-4 lg:p-6 pb-0">
            ...
            <div class="mb-6 relative transition-all duration-500" x-cloak :class="!accountType ? 'blur-[5px] opacity-40 pointer-events-none select-none' : ''">
                <div class="flex justify-between mb-1.5 px-1">
                    <span class="text-[10px] font-black uppercase tracking-[0.2em] transition-colors" :class="currentStep === 1 ? 'text-brand-secondary' : 'text-slate-300'">
                        <?php echo e(app()->isLocale('ar') ? 'بيانات المركز' : 'Center Info'); ?>
                    ...
                    <span class="text-[10px] font-black uppercase tracking-[0.2em] transition-colors" :class="currentStep === 2 ? 'text-brand-secondary' : 'text-slate-300'">
                        <?php echo e(app()->isLocale('ar') ? 'البيانات الشخصية' : 'Personal Details'); ?>
            ...
            <div class="mb-4 text-center transition-all duration-700 ease-in-out" :class="!accountType ? 'transform scale-110 translate-y-[3vh] mb-8' : ''">
                <div x-show="currentStep === 1" x-cloak>
                    <h1 class="text-xl lg:text-2xl font-black text-slate-900 mb-1 font-arabic leading-tight">
                        <?php echo e(app()->isLocale('ar') ? 'ابدأ رحلتك التعليمية' : 'Start Your Journey'); ?>
                    ...
                    <div class="flex items-center justify-center gap-2 mb-2 bg-brand-secondary/5 py-0.5 px-3 rounded-full w-fit mx-auto">
                        ...
                        <span class="text-[9px] font-black text-brand-secondary uppercase tracking-widest" x-text="currentPlan.trial_days > 0 ? (<?php echo e(app()->isLocale('ar') ? 'currentPlan.trial_days + \' يوم مجاناً\'' : 'currentPlan.trial_days + \'-Day Free Trial\''); ?>) : (<?php echo e(app()->isLocale('ar') ? '\'ابدأ الآن\'' : '\'Start Now\''); ?>)"></span>
                    </div>
                    <p class="text-slate-500 text-xs font-arabic font-medium opacity-80 max-w-[280px] mx-auto">
                        <?php echo e(app()->isLocale('ar') ? 'خطوات بسيطة لامتلاك منصتك التعليمية المتكاملة' : 'Simple steps to own your integrated platform'); ?>

                    </p>
                </div>
                <div x-show="currentStep === 2" x-cloak>
                    <h1 class="text-xl lg:text-2xl font-black text-slate-900 mb-1 font-arabic leading-tight">
                        <?php echo e(app()->isLocale('ar') ? 'تأكيد الهوية' : 'Confirm Identity'); ?>

                    </h1>
                    <p class="text-slate-500 text-xs font-arabic font-medium opacity-80">
                        <?php echo e(app()->isLocale('ar') ? 'أدخل بياناتك الشخصية للبدء فوراً' : 'Enter personal details to get started'); ?>
            ...
            <div x-show="currentStep === 1" x-cloak class="mb-4 transition-all duration-700 ease-in-out relative z-10" :class="!accountType ? 'transform scale-110 translate-y-[3vh] pb-12 mt-4' : ''">
                <label class="text-[11px] font-black text-slate-500 px-1 font-arabic uppercase tracking-wider block mb-3 text-center opacity-70 transition-all duration-700" :class="!accountType ? 'text-base text-slate-800 opacity-100 font-black mb-6' : ''">
                    <?php echo e(app()->isLocale('ar') ? 'ابدأ كـ ...' : 'Start as ...'); ?>
                    ...
                    <label class="relative cursor-pointer group">
                        ...
                        <div class="relative flex flex-col items-center justify-center p-4 rounded-2xl border-2 bg-white transition-all duration-500 hover:shadow-xl overflow-hidden group shadow-md"
                            ...
                            <div class="absolute top-0 right-0 bg-brand-secondary text-white text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-bl-lg rounded-tr-xl z-20 shadow-md">
                                <?php echo e(app()->isLocale('ar') ? '30 يوم مجاناً' : '30-Day Free'); ?>
                            ...
                            <span class="font-black transition-all" 
                                  ...
                                  ]"><?php echo e(app()->isLocale('ar') ? 'مدرس مستقل' : 'Independent Tutor'); ?></span>
                    ...
                    <label class="relative cursor-pointer group">
                        ...
                        <div class="relative flex flex-col items-center justify-center p-4 rounded-2xl border-2 bg-white transition-all duration-500 hover:shadow-xl overflow-hidden group shadow-md"
                            ...
                            <span class="font-black transition-all" 
                                  ...
                                  ]"><?php echo e(app()->isLocale('ar') ? 'مركز تعليمي' : 'Educational Center'); ?></span>
            ...
            <div class="mb-6">
                <a :href="'<?php echo e(route('auth.google')); ?>?plan=' + selectedPlan + '&cycle=' + billingCycle + '&account_type=' + (accountType || 'center')" class="w-full flex items-center justify-center gap-2 py-2.5 px-6 border-2 border-slate-200 rounded-2xl shadow-sm text-base font-black text-slate-800 bg-white hover:bg-slate-50 hover:border-blue-500/30 hover:shadow-md transition-all group">
                    ...
                    <span><?php echo e(__('auth.register.google_signup')); ?></span>
                </a>
                <div class="relative my-4 px-8">
                    ...
                    <div class="relative flex justify-center text-[10px] uppercase"><span class="bg-white px-5 text-slate-300 font-bold tracking-[0.2em]"><?php echo e(trans('auth.register.or')); ?></span></div>
                </div>
            </div>

            <form action="<?php echo e(route('register.submit')); ?>" method="POST" class="space-y-4" @submit="if(currentStep === 1) { $event.preventDefault(); nextStep(); }">
                <?php echo csrf_field(); ?>
                ...
                <div x-show="currentStep === 1" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-4">
                    <div class="space-y-1.5">
                        ...
                        <div class="relative group">
                            ...
                            <input type="text" name="center_name" x-model="centerName"
                                ...
                                placeholder="<?php echo e(__('auth.register.center_name_placeholder')); ?>" :required="currentStep === 1">
                        </div>
                    </div>
                    ...
                    <div class="pt-2">
                        <button type="button" @click="nextStep()"
                            ...
                            <span class="text-base font-black font-arabic"><?php echo e(app()->isLocale('ar') ? 'استمرار' : 'Continue'); ?></span>
                ...
                <div x-show="currentStep === 2" x-cloak style="display: none;" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-8 items-start">
                        ...
                        <div class="space-y-4">
                            <div class="space-y-1">
                                <label class="text-[12px] font-black text-slate-400 px-1 font-arabic uppercase tracking-wide"><?php echo e(__('auth.register.full_name')); ?></label>
                            ...
                            <div class="space-y-4">
                                <div class="space-y-1">
                                    <label class="text-[12px] font-black text-slate-400 px-1 font-arabic uppercase tracking-wide"><?php echo e(__('auth.register.email')); ?></label>
                            ...
                            <div class="space-y-1">
                                <div class="flex justify-between items-center px-1">
                                    <label class="text-[12px] font-black text-slate-400 font-arabic uppercase tracking-wide"><?php echo e(__('auth.register.password')); ?></label>
                                    ...
                                    <div x-show="password.length > 0" x-collapse x-cloak class="px-1 py-1">
                                        <div class="flex flex-wrap gap-x-3 gap-y-1.5">
                                            <div class="flex items-center gap-1 text-[10px] font-black font-arabic transition-all duration-300" :class="passwordCriteria.length ? 'text-emerald-500' : 'text-slate-400'">
                                                <i class="bi" :class="passwordCriteria.length ? 'bi-check-circle-fill' : 'bi-circle'"></i> <?php echo e(app()->isLocale('ar') ? '٨ أحرف على الأقل' : '8+ Characters'); ?>
                                            ...
                                            <div class="flex items-center gap-1 text-[10px] font-black font-arabic transition-all duration-300" :class="passwordCriteria.upper ? 'text-emerald-500' : 'text-slate-400'">
                                                <i class="bi" :class="passwordCriteria.upper ? 'bi-check-circle-fill' : 'bi-circle'"></i> <?php echo e(app()->isLocale('ar') ? 'حرف كبير' : 'Uppercase'); ?>
                                            ...
                                            <div class="flex items-center gap-1 text-[10px] font-black font-arabic transition-all duration-300" :class="passwordCriteria.lower ? 'text-emerald-500' : 'text-slate-400'">
                                                <i class="bi" :class="passwordCriteria.lower ? 'bi-check-circle-fill' : 'bi-circle'"></i> <?php echo e(app()->isLocale('ar') ? 'حرف صغير' : 'Lowercase'); ?>
                                            ...
                                            <div class="flex items-center gap-1 text-[10px] font-black font-arabic transition-all duration-300" :class="passwordCriteria.number ? 'text-emerald-500' : 'text-slate-400'">
                                                <i class="bi" :class="passwordCriteria.number ? 'bi-check-circle-fill' : 'bi-circle'"></i> <?php echo e(app()->isLocale('ar') ? 'رقم' : 'Number'); ?>
                                            ...
                                            <div class="flex items-center gap-1 text-[10px] font-black font-arabic transition-all duration-300" :class="passwordCriteria.symbol ? 'text-emerald-500' : 'text-slate-400'">
                                                <i class="bi" :class="passwordCriteria.symbol ? 'bi-check-circle-fill' : 'bi-circle'"></i> <?php echo e(app()->isLocale('ar') ? 'رمز (!@#$)' : 'Symbol (!@#$)'); ?>

                                            </div>
                                        </div>
                                    </div>

                                    <input :type="showPassword ? 'text' : 'password'" name="password_confirmation" x-model="password_confirmation" class="w-full h-11 px-5 bg-slate-50/50 border-2 border-slate-100 rounded-2xl text-base font-bold focus:outline-none focus:bg-white focus:ring-4 focus:ring-brand-secondary/5 focus:border-brand-secondary transition-all shadow-inner" :class="password_confirmation.length > 0 && !isPasswordMatch ? 'border-red-300 bg-red-50' : ''" placeholder="<?php echo e(__('auth.register.confirm_password')); ?>" :required="currentStep === 2">
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="p-4 rounded-2xl bg-slate-50/80 border border-slate-100 relative overflow-hidden">
                                ...
                                <div class="flex p-1 bg-slate-200/50 rounded-xl mb-3 items-center">
                                    <button type="button" @click="billingCycle = 'monthly'" 
                                        ...
                                        <?php echo e(app()->getLocale() == 'ar' ? 'شهري' : 'Monthly'); ?>
                                ...
                                <div class="flex items-center justify-between gap-2 opacity-60">
                                    <div class="flex items-center gap-1 text-slate-500 text-[9px] font-bold font-arabic">
                                        ...
                                        <span><?php echo e(app()->getLocale() == 'ar' ? 'دفع آمن' : 'Secure'); ?></span>
                                    </div>
                                    <div class="flex items-center gap-1 text-slate-500 text-[9px] font-bold font-arabic">
                                        <i class="bi bi-arrow-repeat text-brand-secondary"></i>
                                        <span><?php echo e(app()->getLocale() == 'ar' ? 'إلغاء مرن' : 'Flexible'); ?></span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="pt-4 flex flex-col sm:flex-row gap-3">
                        <button type="button" @click="prevStep()" class="flex-1 h-12 rounded-full font-black text-slate-500 bg-slate-50 hover:bg-slate-100 transition-all border-2 border-slate-100">
                            <?php echo e(app()->isLocale('ar') ? 'رجوع' : 'Back'); ?>
                        ...
                        <button type="submit" :disabled="password.length > 0 && !isPasswordMatch"
                            ...
                            <span class="relative z-10" x-text="currentPlan.trial_days > 0 ? (<?php echo e(Js::from(app()->isLocale('ar') ? 'ابدأ الفترة التجريبية' : 'Start Free Trial')); ?>) : (finalPrice === 0 ? '<?php echo e(__('auth.register.cta_main')); ?>' : '<?php echo e(app()->isLocale('ar') ? 'ادفع واستكمل التسجيل' : 'Pay & Complete'); ?>')"></span>
                        </button>
                    </div>
                    ...
                    <div class="mt-4 flex flex-col items-center gap-2">
                        <div class="flex items-center justify-center gap-2 text-slate-600 bg-emerald-50/50 px-4 py-2 rounded-xl border border-emerald-100/50 w-full text-center">
                            ...
                            <p class="text-[11px] font-bold font-arabic"><?php echo e(app()->getLocale() == 'ar' ? 'ضمان استرجاع الأموال خلال 30 يوماً.' : '30-Day Money-Back Guarantee.'); ?></p>
                        </div>
                    </div>
                </div>
        ...
        <div class="p-4 bg-slate-50/50 border-t border-slate-100 text-center transition-all duration-500" :class="!accountType ? 'blur-[5px] opacity-40 pointer-events-none select-none' : ''">
            <span class="text-xs text-slate-500 font-arabic font-bold">
                ...
                <a href="<?php echo e(route('login.portal')); ?>" class="text-brand-secondary font-black hover:underline ml-2"><?php echo e(__('auth.login.title')); ?></a>
            </span>
        </div>
